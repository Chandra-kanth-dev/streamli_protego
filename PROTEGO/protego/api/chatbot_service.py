...
def handle_message(
    ...
    # Quick bypass for simple greetings to prevent classifier false-positives
    greetings_pattern = re.compile(r"^(hello|hi|hey|greetings|how are you|good (morning|afternoon|evening))\b", re.IGNORECASE)
    cleaned_msg = message.strip().lower().rstrip("?.! ")
    if greetings_pattern.match(cleaned_msg) and len(cleaned_msg.split()) <= 6:
        if not any(w in cleaned_msg for w in ["hurt", "kill", "unsafe", "scared", "hit", "beat", "abuse", "help", "threat"]):
            return {
                "reply": "Hello 🤍 I'm here for you. Whatever you're going through, you don't have to face it alone. How can I help you today?",
                "risk_level": "low",
                "emotion": "neutral",
                "sentiment": "neutral",
                "show_emergency": False,
    ...
    if context_memory is None:
        context_memory = _context_memory
    ...
    # -----------------------------
    # 3. Predictions (with Out-of-Vocabulary Fallbacks)
    # -----------------------------
    if X_emotion.nnz == 0:
        emotion = "neutral"
    else:
    ...
    risk_result = compute_risk(
        ...
        sentiment=sentiment,
        ml_risk=ml_risk,
        previous_risks=context_memory.get_recent_risks()
    )
    ...
    safety = apply_safety_rules(
        ...
        context_summary=context_memory.summary()
    )
    ...
    # 6. Update context
    # -----------------------------
    context_memory.update(risk=final_risk, emotion=emotion)
